...
while(have_posts()) {
  ...
  <?php
    ...
    ?>
    <?php 
    $testChildren = get_pages(array(
        'child_of' => get_the_ID()
    ));

    if ($theParent or $testChildren) { ?>
    <div class="page-links">
      <h2 class="page-links__title"><a href="<?php echo get_permalink($theParent);?>"><?php echo get_the_title($theParent); ?></a></h2>
      <ul class="min-list">
         <?php 
         if ($theParent) {
            $findCholdrenOf = $theParent;   
         } else {
                $findCholdrenOf = get_the_ID();
         }

         wp_list_pages(array(
             'title_li' => NULL,
             'child_of' => $findCholdrenOf,
